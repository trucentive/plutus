  ...
  module AmountsExtension
    ...
    def balance(hash={})
      if hash[:from_date] && hash[:to_date]
        from_date = hash[:from_date].kind_of?(DateTime)||hash[:from_date].kind_of?(Date) ? hash[:from_date] : DateTime.strptime(hash[:from_date],"%Y-%m-%d")
        to_date = hash[:to_date].kind_of?(DateTime) || hash[:to_date].kind_of?(Date) ? hash[:to_date] : DateTime.strptime(hash[:to_date],"%Y-%m-%d")
